...
CREATE TABLE terminal_session_commands (
    id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE terminal_session_environments (
    terminal_session_id INTEGER NOT NULL,
    name TEXT NOT NULL,
    value TEXT NOT NULL,
    FOREIGN KEY (terminal_session_id) REFERENCES terminal_sessions(id) ON DELETE CASCADE
) STRICT WITHOUT ROWID;
CREATE UNIQUE INDEX terminal_session_environments_name_index
ON terminal_session_environments(terminal_session_id, name);

-- Table which stores the overall command history
...
CREATE UNIQUE INDEX terminal_commands_command_index
ON terminal_commands(command);

-- Table is automatically updated when a new command is inserted or updated
CREATE TRIGGER insert_terminal_command
AFTER INSERT ON terminal_session_commands
WHEN new.command IS NOT NULL
BEGIN
    INSERT INTO terminal_commands(command, number_of_uses, last_used)
...
CREATE VIRTUAL TABLE terminal_commands_fts
USING fts5(
    command,
    tokenize='trigram',
...
CREATE TRIGGER insert_terminal_commands_fts
AFTER INSERT ON terminal_commands_fts
BEGIN
    INSERT INTO terminal_commands_fts(rowid, command)
    VALUES (new.id, new.command);
END;

CREATE TRIGGER delete_terminal_commands_fts
AFTER DELETE ON terminal_commands_fts
BEGIN
    INSERT INTO delete_terminal_commands_fts(terminal_command_history_fts, rowid, command)
